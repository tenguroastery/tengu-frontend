...
export default function Stars({ rating, size = 'md', className = '' }: Props) {
  ...
  // useId garantiza ids únicos cuando hay múltiples <Stars> en la misma página
  // (sino el primer linearGradient pisa a los demás y se pintan transparentes).
  ...
  return (
    ...
    >
      {[1, 2, 3, 4, 5].map((i) => {
        ...
        const gradId = `${gradientPrefix}-half-${i}`;
        return (
          <svg
            key={i}
            width={px}
          ...
          >
            ...
            <path
              ...
              fill={filled ? '#C8842A' : half ? `url(#${gradId})` : 'rgba(15,15,15,0.15)'}
            />
          </svg>
        );
      })}
